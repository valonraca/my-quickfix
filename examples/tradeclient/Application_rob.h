...
extern FIX::SenderCompID senderCompId;
extern FIX::TargetCompID targetCompId;
...
{
public:
  Application(const FIX::SessionSettings& s) {
    auto sessionIds = s.getSessions();
    if ( sessionIds.empty() ) {
      throw std::runtime_error{"Could not find session id"};
    }
    auto it = sessionIds.begin();
    senderCompId = it->getSenderCompID();
    targetCompId = it->getTargetCompID();
  }
  void run();
...
private:
  ...
  void queryResendRequest();
  void queryFailedOrders();
  void queryLoopOrders();
  void querySpecialOrders();
  void queryMarketSubscription();
